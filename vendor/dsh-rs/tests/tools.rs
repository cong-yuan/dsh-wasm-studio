...
fn make_registry() -> (cordis::Context, ToolRegistry) {
    ...
    dsh_rs::tools::builtin::register_builtin_tools(&registry).unwrap();
    (ctx, registry)
}
...
async fn guard_denies_after_waterfall() {
    ...
    let registry = ToolRegistry::new(ctx.clone());
    dsh_rs::tools::builtin::register_builtin_tools(&registry).unwrap();
    registry.add_guard(Arc::new(|args: &dsh_rs::tools::ToolCallArgs| {
...
async fn tools_plugin_provides_registry() {
    ...
    let registry = ctx.require::<dsh_rs::api::services::ToolsService>("tools").unwrap();
