...
use crate::llm::runtime::{BoxStream, LlmAdapter};
use crate::types::{
...
fn parse_event(event: &Value) -> StreamChunk {
    ...
    // 9router (and some gateways) emit finish_reason:"" on non-terminal
    // deltas. Treat only non-empty reasons as terminal — empty would make the
    // assembler finalize on the first SSE event and drop all later text.
