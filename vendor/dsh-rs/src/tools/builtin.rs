...
use crate::tools::matcher::glob_match;
use crate::tools::registry::{ToolDefinition, ToolRegistry};
use crate::tools::{ToolCallArgs, ToolExecutionResult, ToolRunContext};
    ...
    Arc::new(ToolDefinition::new(
        ...
        |args: ToolCallArgs, run_ctx: ToolRunContext| {
            Box::pin(async move {
                ...
                let Some(sessions) = run_ctx.ctx.get::<crate::api::services::SessionService>("sessions")
                else {
                ...
                let todos: Vec<crate::types::TodoItem> = parsed
                    .get("todos")
                    .and_then(|t| t.as_array())
                    .map(|arr| {
                        arr.iter()
                            .map(|entry| crate::types::TodoItem {
                                content: entry
                                    .get("content")
                                    .and_then(|c| c.as_str())
                                    .unwrap_or("")
                                ...
                                status: match entry
                                    ...
                                    .and_then(|s| s.as_str())
                                {
                                    Some("in_progress") => crate::types::TodoStatus::InProgress,
                                    Some("completed") => crate::types::TodoStatus::Completed,
                                    _ => crate::types::TodoStatus::Pending,
                                },
                            })
                            .collect()
                    })
                    .unwrap_or_default();
                session.append(crate::types::SessionEventData::TodoWrite { todos: todos.clone() });
